...
void b2World_Step_original( b2WorldId worldId, float timeStep, int subStepCount )
{
...
{
	// enable_detailed_verification_output();

	PROOF_CONTEXT("b2World_Step");
	
	// Function contract
	CONTRACT_BEGIN("b2World_Step");
	PRECONDITION_VOID( "timeStep is valid float", require_bool( "timeStep is finite", isfinite( timeStep ) ) );
	PRECONDITION_VOID( "subStepCount is positive", require_bool( "subStepCount > 0", subStepCount > 0 ) );
	...
	if (!require_bool("timeStep is valid float", b2IsValidFloat(timeStep))) {
		PROOF_CONTEXT_END();
		CONTRACT_END();
		return;
	}
	if (!require_bool("subStepCount is positive", 0 < subStepCount)) {
		PROOF_CONTEXT_END();
		CONTRACT_END();
		return;
	}

	// World retrieval and validation
	b2World* world = b2GetWorldFromId(worldId);
	if (!require_bool("world is valid", world != NULL)) {
		PROOF_CONTEXT_END();
		CONTRACT_END();
		return;
	}
	if (!require_bool("world is not locked", world->locked == false)) {
		PROOF_CONTEXT_END();
		CONTRACT_END();
		return;
	}
	...
	// Event array management with verification
	{
		PROOF_CONTEXT("event_array_preparation");
		...
		// Verify event arrays are properly cleared
		require_bool("body move events cleared", world->bodyMoveEvents.count == 0);
		require_bool("sensor begin events cleared", world->sensorBeginEvents.count == 0);
		require_bool("contact begin events cleared", world->contactBeginEvents.count == 0);
		require_bool("contact hit events cleared", world->contactHitEvents.count == 0);
	...
	// Handle zero timestep case
	if (timeStep == 0.0f) {
		PROOF_CONTEXT("zero_timestep_handling");
		
		// Verify timestep is exactly zero
		require_bool("timestep is zero", timeStep == 0.0f);
		
		// Swap end event array buffers
		int oldIndex = world->endEventArrayIndex;
		world->endEventArrayIndex = 1 - world->endEventArrayIndex;
		
		// Verify buffer index is valid
		require_bool("end event array index valid", 
		            world->endEventArrayIndex >= 0 && world->endEventArrayIndex <= 1);
		require_bool("buffer index changed", world->endEventArrayIndex != oldIndex);
		
		b2SensorEndTouchEventArray_Clear(world->sensorEndEvents + world->endEventArrayIndex);
		b2ContactEndTouchEventArray_Clear(world->contactEndEvents + world->endEventArrayIndex);

		// Verify end event arrays are cleared
		require_bool("sensor end events cleared", 
		            world->sensorEndEvents[world->endEventArrayIndex].count == 0);
		require_bool("contact end events cleared", 
		            world->contactEndEvents[world->endEventArrayIndex].count == 0);
	...
	// Begin main simulation step
	b2TracyCZoneNC(world_step, "Step", b2_colorBox2DGreen, true);

	// Lock world and initialize task counters
	{
		PROOF_CONTEXT("world_state_initialization");
		...
		// Verify world state
		require_bool("world is now locked", world->locked == true);
		require_bool("active task count reset", world->activeTaskCount == 0);
		require_bool("task count reset", world->taskCount == 0);
	...
	// Update collision pairs and create contacts
	{
		PROOF_CONTEXT("broad_phase_collision");
		
		uint64_t pairTicks = b2GetTicks();
		
		// Verify tick values are reasonable
		require_bool("pair ticks valid", pairTicks > 0);
		require_bool("step ticks valid", stepTicks > 0);
		require_bool("tick ordering", pairTicks >= stepTicks);
		
		b2UpdateBroadPhasePairs(world);
		
		uint64_t pairEndTicks = b2GetTicks();
		require_bool("pair end ticks valid", pairEndTicks >= pairTicks);
		
		world->profile.pairs = b2GetMilliseconds(pairTicks);
		require_bool("pairs profile time valid", world->profile.pairs >= 0.0f);
		
		PROOF_CONTEXT_END();
	}

	// Setup step context with comprehensive verification
	{
		PROOF_CONTEXT("step_context_setup");
		...
		// Verify basic context setup
		require_bool("context world valid", context.world == world);
		require_bool("context dt matches", context.dt == timeStep);
		require_bool("context substep count valid", context.subStepCount >= 1);
		require_bool("context substep count reasonable", context.subStepCount <= 1000);

		// Calculate timing parameters with verification
		if (timeStep > 0.0f) {
			PROOF_CONTEXT("positive_timestep_calculations");
			
			require_bool("timestep is positive", timeStep > 0.0f);
			require_bool("timestep is reasonable", timeStep < 1.0f); // Sanity check
			
			context.inv_dt = 1.0f / timeStep;
			require_bool("inv_dt is finite", isfinite(context.inv_dt));
			require_bool("inv_dt is positive", context.inv_dt > 0.0f);
			
			context.h = timeStep / context.subStepCount;
			require_bool("h is finite", isfinite(context.h));
			require_bool("h is positive", context.h > 0.0f);
			require_bool("h is reasonable", context.h <= timeStep);
			
			context.inv_h = context.subStepCount * context.inv_dt;
			require_bool("inv_h is finite", isfinite(context.inv_h));
			require_bool("inv_h is positive", context.inv_h > 0.0f);
			
			// Verify mathematical relationships
			require_bool("dt * inv_dt approximately 1", 
			            fabs(timeStep * context.inv_dt - 1.0f) < 0.001f);
			require_bool("h * subStepCount approximately dt", 
			            fabs(context.h * context.subStepCount - timeStep) < 0.001f);
			
			PROOF_CONTEXT_END();
		} else {
			PROOF_CONTEXT("zero_timestep_calculations");
			...
			require_bool("inv_dt is zero", context.inv_dt == 0.0f);
			require_bool("h is zero", context.h == 0.0f);
			require_bool("inv_h is zero", context.inv_h == 0.0f);
			
			PROOF_CONTEXT_END();
		}

		world->inv_h = context.inv_h;
		require_bool("world inv_h updated", world->inv_h == context.inv_h);

		// Contact and softness calculations with verification
		{
			PROOF_CONTEXT("contact_softness_calculations");
			
			// Hertz values get reduced for large time steps
			float contactHertz = b2MinFloat(world->contactHertz, 0.125f * context.inv_h);
			
			require_bool("contact hertz is finite", isfinite(contactHertz));
			require_bool("contact hertz is positive", contactHertz > 0.0f);
			require_bool("contact hertz is reasonable", contactHertz <= world->contactHertz);
			
			context.contactSoftness = b2MakeSoft(contactHertz, world->contactDampingRatio, context.h);
			context.staticSoftness = b2MakeSoft(2.0f * contactHertz, world->contactDampingRatio, context.h);
			
			// Verify softness values
			require_bool("contact softness valid", 
			            isfinite(context.contactSoftness.massScale) && 
			            isfinite(context.contactSoftness.impulseScale));
			require_bool("static softness valid", 
			            isfinite(context.staticSoftness.massScale) && 
			            isfinite(context.staticSoftness.impulseScale));
			
			require_bool("contact softness mass scale positive", 
			            context.contactSoftness.massScale > 0.0f);
			require_bool("static softness mass scale positive", 
			            context.staticSoftness.massScale > 0.0f);

			world->contactSpeed = world->maxContactPushSpeed / context.staticSoftness.massScale;
			
			require_bool("contact speed is finite", isfinite(world->contactSpeed));
			require_bool("contact speed is positive", world->contactSpeed > 0.0f);
			
			PROOF_CONTEXT_END();
		}

		// Setup remaining context parameters
		context.restitutionThreshold = world->restitutionThreshold;
		context.maxLinearVelocity = world->maxLinearSpeed;
		context.enableWarmStarting = world->enableWarmStarting;

		require_bool("restitution threshold valid", isfinite(context.restitutionThreshold));
		require_bool("max linear velocity valid", isfinite(context.maxLinearVelocity));
		require_bool("max linear velocity positive", context.maxLinearVelocity > 0.0f);

		// Update contacts with verification
		{
			PROOF_CONTEXT("contact_update");
			
			uint64_t collideTicks = b2GetTicks();
			require_bool("collide ticks valid", collideTicks > 0);
			
			b2Collide(&context);
			
			uint64_t collideEndTicks = b2GetTicks();
			require_bool("collide end ticks valid", collideEndTicks >= collideTicks);
			
			world->profile.collide = b2GetMilliseconds(collideTicks);
			require_bool("collide profile time valid", world->profile.collide >= 0.0f);
		...
		// Integrate velocities, solve velocity constraints, and integrate positions
		if (context.dt > 0.0f) {
			PROOF_CONTEXT("physics_solving");
			
			require_bool("solving with positive dt", context.dt > 0.0f);
			
			uint64_t solveTicks = b2GetTicks();
			require_bool("solve ticks valid", solveTicks > 0);
			
			b2Solve(world, &context);
			
			uint64_t solveEndTicks = b2GetTicks();
			require_bool("solve end ticks valid", solveEndTicks >= solveTicks);
			
			world->profile.solve = b2GetMilliseconds(solveTicks);
			require_bool("solve profile time valid", world->profile.solve >= 0.0f);
		...
		// Update sensors
		{
			PROOF_CONTEXT("sensor_update");
			
			uint64_t sensorTicks = b2GetTicks();
			require_bool("sensor ticks valid", sensorTicks > 0);
			
			b2OverlapSensors(world);
			
			uint64_t sensorEndTicks = b2GetTicks();
			require_bool("sensor end ticks valid", sensorEndTicks >= sensorTicks);
			
			world->profile.sensors = b2GetMilliseconds(sensorTicks);
			require_bool("sensors profile time valid", world->profile.sensors >= 0.0f);
	...
	// Finalize timing and memory verification
	{
		PROOF_CONTEXT("finalization");
		
		uint64_t stepEndTicks = b2GetTicks();
		require_bool("step end ticks valid", stepEndTicks >= stepTicks);
		
		world->profile.step = b2GetMilliseconds(stepTicks);
		require_bool("step profile time valid", world->profile.step >= 0.0f);

		// Verify memory arena is clean
		size_t arenaAllocation = b2GetArenaAllocation(&world->arena);
		if (!require_bool("arena allocation is zero", arenaAllocation == 0)) {
			// This is a critical error - memory leak detected
			PROOF_CONTEXT_END();
			PROOF_CONTEXT_END();
			CONTRACT_END();
			return;
		...
		// Make sure all tasks that were started were also finished
		if (!require_bool("all tasks completed", world->activeTaskCount == 0)) {
			// This is a critical error - threading issue detected
			PROOF_CONTEXT_END();
			PROOF_CONTEXT_END();
			CONTRACT_END();
			return;
	...
	// Final cleanup with verification
	{
		...
		world->locked = false;
		require_bool("world is unlocked", world->locked == false);
		
		PROOF_CONTEXT_END();
	}

	// Postconditions
	POSTCONDITION_VOID("world is unlocked", require_bool("world unlocked", world->locked == false));
	POSTCONDITION_VOID( "no active tasks", require_bool( "no tasks active", world->activeTaskCount == 0 ) );
	POSTCONDITION_VOID( "clean memory arena", require_bool( "arena clean", b2GetArenaAllocation( &world->arena ) == 0 ) );
